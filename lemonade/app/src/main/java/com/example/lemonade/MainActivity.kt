...
class MainActivity : AppCompatActivity() {
    ...
    private var mLemonadeState = "select"
    // Default lemonSize to -1
    private var mLemonSize = -1
    // Default the squeezeCount to -1
    private var mSqueezeCount = -1

    private var mLemonTree = LemonTree()
    private var mLemonImage: ImageView? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            mLemonadeState = savedInstanceState.getString(LEMONADE_STATE, "select")
            mLemonSize = savedInstanceState.getInt(LEMON_SIZE, -1)
            mSqueezeCount = savedInstanceState.getInt(SQUEEZE_COUNT, -1)
        }
        // === END IF STATEMENT ===

        mLemonImage = findViewById(R.id.image_lemon_state)
        setViewElements()
        mLemonImage!!.setOnClickListener {
            // TODO: call the method that handles the state when the image is clicked
            clickLemonImage()
        }
        mLemonImage!!.setOnLongClickListener {
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        outState.putString(LEMONADE_STATE, mLemonadeState)
        outState.putInt(LEMON_SIZE, mLemonSize)
        outState.putInt(SQUEEZE_COUNT, mSqueezeCount)
    ...
    private fun clickLemonImage() {
        ...
        when(mLemonadeState) {
            ...
            SELECT -> {
                mLemonSize = mLemonTree.pick()
                mSqueezeCount = 0
                mLemonadeState = SQUEEZE
            }
    ...
    private fun setViewElements() {
        ...
        when(mLemonadeState) {
            SELECT -> {
                ...
                mLemonImage?.setImageResource(R.drawable.lemon_tree)
            }
            SQUEEZE -> {
                textAction.setText(R.string.lemon_squeeze)
                mLemonImage?.setImageResource(R.drawable.lemon_squeeze)
            }
            DRINK -> {
                textAction.setText(R.string.lemon_drink)
                mLemonImage?.setImageResource(R.drawable.lemon_drink)
            }
            RESTART -> {
                textAction.setText(R.string.lemon_empty_glass)
                mLemonImage?.setImageResource(R.drawable.lemon_restart)
            }
        }
    ...
    private fun showSnackbar(): Boolean {
        if (mLemonadeState != SQUEEZE) {
            return false
        }
        val squeezeText = getString(R.string.squeeze_count, mSqueezeCount)
        Snackbar.make(
